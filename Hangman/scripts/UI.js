class UI {
        ...
        return `
            <h4 id="enterGuessTitle" class="subTitle">Enter Guess</h4>
             ...
             <h4 id="lettersGuessed" class="subTitle">Letters Guessed So Far</h4>
                <div id="lettersGuessedSoFar">${this.separateLettersToSpanElements((' '))}</div>

             <h4 class="subTitle">Guessed Correctly</h4>
              <div id="renderLetters">${this.separateLettersToSpanElements(this.upperCaseFirstLetter(gameState.wordSoFar))}</div>

              <h4 class="subTitle">Remaining Guesses</h4>
                <span id='guessesRemaining' class="puzzleLetters">${gameState.remainingGuesses}</span>

             <h4 class="subTitle">Difficulty Level</h4>
                <p class="puzzleLetters"> ${gameState.difficulty} </p>
             `
     }
    ...
    updateDomGameRender(gameState) {
        ...
        renderLettersElement.innerHTML = this.separateLettersToSpanElements(this.upperCaseFirstLetter(gameState.wordSoFar))
        document.querySelector('#guessesRemaining').textContent = remainingGuesses
        document.querySelector('#lettersGuessedSoFar').innerHTML = this.separateLettersToSpanElements((allLettersGuessed ? allLettersGuessed : '*'))
    }

    updateDomGameFinishedRender(gameState, result) {
      const hangmanInfoElem = document.querySelector('#hangmanInfo');
      hangmanInfoElem.innerHTML = this.generateHTMLForFinishedGame(gameState, result) 
  }
    ...
    generateHTMLForFinishedGame(gameState, result) {
      ...
      return `
          <h3>${result}</h3>
          <h4 id="Correct Word" class="subTitle">Correct Word</p>
              <div>${this.separateLettersToSpanElements(this.upperCaseFirstLetter(gameWord))}</div>
              
          <h4 id="renderLetters" class="subTitle">Guessed Correctly</h4>
              <div>${this.separateLettersToSpanElements(this.upperCaseFirstLetter(wordSoFar))}</div>

          <h4 class="subTitle">Remaining Guesses</h4>
              <span id="guessesRemaining" class='puzzleLetters'>${remainingGuesses}</span>

          <h4 class="subTitle">Difficulty Level</h4>
            <span class='puzzleLetters'>${difficulty}</span>
          `
  }
